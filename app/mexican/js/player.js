...
Player.checkCubeCollision = function(map,x,z) {
    ...
    if (xToUse < 0 || zToUse < 0 || zToUse >= map.length || xToUse >= map[zToUse].length
        || map[zToUse][xToUse].id == 0 ||  map[zToUse][xToUse].id == 3 || map[zToUse][xToUse].id == 6  || map[zToUse][xToUse].id == 9 
        || map[zToUse][xToUse].id == 10 || (map[zToUse][xToUse].id == 11 && !Map.magicDoorsOpen) ) {
        return true;
    }
    else {
        return false;
